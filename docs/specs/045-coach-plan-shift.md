...
- **Status: Draft**（David 2026-09-02 grill 拍板：1A 2不做 3A 4赞同 5同意 6进；入口形态待设计稿拍板后转 Approved）
...
- **A（推荐）日头入口**：选中天（`sel`）的 `.dayhead` 悬停/选中时出现「后移…」小按钮；点击弹出锚定在该列下方的弹层。锚定日 = 该天当前推荐日期（含既有后移）。
- **B 顶栏入口**：`TopBar` 在 `StartDateControl` 右侧加「后移…」按钮，弹层内自带日期选择（默认 = 选中天的推荐日期，未选中则 = 今天）。
- 两案弹层内容相同：标题「从 M/D（W#D#）起后移」；步进器 `− N +`（默认 1，范围 1–30，可直接输入）；预览行「W#D# → M/D 周X · 影响 K 个训练日，已完成的 J 天不动」（K/J 由本地计算：推荐日期 ≥ 锚定日的天中，`completed_at` 非空者为 J）；提示行「学员端会自动更新并收到通知」；按钮「取消」/「后移 N 天」（主按钮沿 `apply` 样式）。
...
- `ShiftBadge` 文案「顺延」→「已后移」；tooltip「原定 M/D · 已后移 N 天（教练 / 学员）」。i18n zh/en 同步（`shift` 词根，勿用 `postpone`）。
