...
function jumpToWeek(host: HTMLElement, weekNumber: number): void {
  ...
  act(() => click(jump))
  const target = [...host.querySelectorAll<HTMLElement>('.popitem')]
    .find((element) => element.textContent === `W${String(weekNumber).padStart(2, '0')}`)!
  act(() => click(target))
}
...
describe('plan editor multi-day selection UI', () => {
  ...
  it('batch-clears editable rows in mixed logged days and keeps the logged rows', () => {
    ...
    act(() => click(contextAction(host, '设为休息(3 天)')))
    expect(day(host, 1, 0).className).toContain('restday')
    expect(day(host, 1, 1).className).not.toContain('restday')
    expect(day(host, 1, 2).className).toContain('restday')
    expect(host.querySelector('.plan-autosave-status')?.textContent)
      .toContain('已设为休息 2 天，1 天含学员已打卡已跳过')
  })

  it('batch-unsets rest on logged days and only counts days that actually changed', () => {
    const data = week(1, {
      0: [row('logged-rest', '已打卡动作', { hasLogs: true })],
      2: [row('already-training', '训练动作')],
    })
    ...
    act(() => click(contextAction(host, '改为训练日(2 天)')))

    expect(day(host, 1, 0).className).not.toContain('restday')
    expect(day(host, 1, 0).querySelector('[data-rowid="logged-rest"]')).not.toBeNull()
    expect(day(host, 1, 2).className).not.toContain('restday')
    expect(host.querySelector('.plan-autosave-status')?.textContent).toContain('已改为训练日 1 天')
  })
  ...
  it('keeps ordinary single-day labels, controls, and no-confirm clear behavior unchanged', () => {
    ...
    expect(host.querySelector('[data-selected-context] b')?.textContent).toBe('周一 8/3（第 1 周）')
    expect(contextAction(host, '设为休息')).not.toBeNull()
