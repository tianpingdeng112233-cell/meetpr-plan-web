...
describe('parseSetLine', () => {
  ...
  it('treats amrap and 力竭 as AMRAP: reps plus when a base count exists, RPE 10 loads', () => {
    ...
    // A base count makes the to-failure set an AMRAP "12+", not a plain "12".
    expect(failure.reps).toBe('12+')
    expect(failure.mode).toBe('rpe')
    expect(failure.values).toEqual(['9', '9'])
    // Encoded into reps, so the redundant 力竭 note is dropped.
    expect(failure.note).not.toContain('力竭')
    // No base count: never fake a "1" rep. Leave reps unset and keep 力竭 so the
    // to-failure meaning survives for the coach/student.
  ...
  it('keeps explicit bodyweight failure rows in bodyweight mode', () => {
    // Bodyweight to-failure: no fake "1" rep, and 力竭 stays in the note since
    // there is no RPE load or base count to carry the to-failure meaning.
